...
class E0509SceneCfg(InteractiveSceneCfg):
    ...
    block = RigidObjectCfg(
        ...
        spawn=sim_utils.MeshCuboidCfg(
            size=(0.05, 0.05, 0.05),  # 5cm 큐브
            rigid_props=sim_utils.RigidBodyPropertiesCfg(
                disable_gravity=False,  # 중력 활성화
            ),
            mass_props=sim_utils.MassPropertiesCfg(mass=0.1),
            collision_props=sim_utils.CollisionPropertiesCfg(
                collision_enabled=True,  # 콜리젼 활성화
            ),
        ...
        init_state=RigidObjectCfg.InitialStateCfg(pos=(0.6, 0.0, 0.025)),  # 바닥 위 2.5cm (블록 중심) - 로봇에서 더 멀리
    )
    ...
    # 🆕 Contact sensor for gripper-block interaction
    ...
    # 🚨 NEW: Self-collision and ground collision detection
    collision_sensor = ContactSensorCfg(
        ...
        filter_prim_paths_expr=[
            "{ENV_REGEX_NS}/Robot/.*",  # 자기 충돌 감지
            "/World/defaultGroundPlane"  # 땅과의 충돌 감지
        ],
    )
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        # object positions
        block_pos = ObsTerm(func=mdp.root_pos_w, params={"asset_cfg": SceneEntityCfg("block")})
        ...
        # relative distances
        rel_ee_block_distance = ObsTerm(func=custom_mdp.rel_ee_block_distance)
...
class RewardsCfg:
    ...
    # 🎯 NEW: Gripper Alignment Reward
    gripper_alignment = RewTerm(
        func=custom_mdp.gripper_alignment_reward,
        weight=CONFIG.weights.GRIPPER_ALIGNMENT,
        params={
            "gripper_cfg": SceneEntityCfg("robot", body_names=CONFIG.gripper.FINGER_TIPS),  # 끝단으로 정확한 위치 측정
            "target_cfg": SceneEntityCfg("block"),
            "xy_threshold": CONFIG.distance.CLOSE_APPROACH,
            "height_threshold": CONFIG.height.ALIGNMENT_HEIGHT,  # 5cm 높이로 블록 위에 정렬
            "orientation_weight": CONFIG.weights.ORIENTATION_WEIGHT,
        },
    )
    ...
    # 🚨 NEW: Safety Penalties (Critical!)
    collision_penalty = RewTerm(
        func=custom_mdp.collision_penalty,
        weight=1.0,  # Full weight - this is critical!
        params={
            "robot_cfg": SceneEntityCfg("robot"),
            "collision_sensor_name": "collision_sensor",
            "self_collision_penalty": CONFIG.penalty.SELF_COLLISION,
            "ground_collision_penalty": CONFIG.penalty.GROUND_COLLISION,
        },
    )
...
class EventsCfg:
    ...
    reset_block = EventTerm(
        ...
        params={
            ...
            "pose_range": {
                "x": (0.45, 0.75),  # 로봇에서 더 멀리 (0.6 중심으로 범위 설정)
                "y": (-0.4, 0.4),  # Y 범위는 그대로 유지
                "z": (0.025, 0.025),  # 바닥 위 2.5cm (블록 중심)
            },
            # 🔧 속도 범위 제거 - 블록이 자연스럽게 물리 법칙 적용되도록
        },
    )
