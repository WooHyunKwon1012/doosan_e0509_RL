...
usd_path = "/home/woo/Downloads/source/3d/Jiwoo_e0509_gripper/e0509_model.usd"
robot_prim_path = "/World/e0509_model"
add_reference_to_stage(usd_path=usd_path, prim_path=robot_prim_path)
my_world.scene.add_default_ground_plane()

# Gripper 체크 (e0509_with_gripper)
print("\n========== [e0509_with_gripper] Checking Gripper ==========")
from pxr import Usd
stage = my_world.stage
gripper_paths = [
    "/World/e0509_model/e0509/gripper",
    "/World/e0509_model/gripper",
]
for gpath in gripper_paths:
    prim = stage.GetPrimAtPath(gpath)
    print(f"{gpath}: Valid={prim.IsValid()}")
    if prim.IsValid():
        print(f"  Type: {prim.GetTypeName()}")
        children = list(prim.GetChildren())
        print(f"  Children count: {len(children)}")
        if len(children) > 0:
            print(f"  First 5 children: {[c.GetPath() for c in children[:5]]}")
