...
try:
    rb_api = UsdPhysics.RigidBodyAPI.Apply(cube_root_prim)
    rb_api.CreateRigidBodyEnabledAttr().Set(True)
except: pass
try: PhysxSchema.PhysxRigidBodyAPI.Apply(cube_root_prim)
except: pass
try:
    mass_api = UsdPhysics.MassAPI.Apply(cube_root_prim)
...
# ------------------------------
# ROS2 Joint Publisher Node
# ------------------------------
class JointPubNode(Node):
    def __init__(self, robot, num_dof):
        super().__init__("e0509_joint_pub")
        self.robot = robot
        self.num_dof = num_dof
        self.pub = self.create_publisher(Float32MultiArray, "/joint_pos", 10)
        self.get_logger().info(
            f"JointPubNode started. Publishing /joint_pos. DOF={num_dof}"
        )
...
# ROS2 init 및 노드 생성
rclpy.init()
joint_node = JointPubNode(robot=robot, num_dof=num_joints)
...
def teleport_rigid(dc, handle_or_path, pos, quat_wxyz):
    ok = False
    try:
        dc.set_rigid_body_world_pose(handle_or_path,list(pos),list(quat_wxyz)); ok=True
    except:
        pass
    try:
        dc.set_rigid_body_linear_velocity(handle_or_path,[0,0,0])
    except:
        pass
    try:
        dc.set_rigid_body_angular_velocity(handle_or_path,[0,0,0])
    except:
        pass
    try:
        dc.wake_up_rigid_body(handle_or_path)
    except:
        pass
    return ok
...
def reset_episode():
    ...
    if dc:
        teleport_rigid(dc,target,[x,y,z],[1,0,0,0])
    prev_action = np.zeros(action_dim,dtype=np.float32)
    world.step(render=False)
    ...
    try:
        while simulation_app.is_running():
            ...
            if not world.is_playing():
                continue
            if first_play:
                translate_op.Set(translate_op.Get())
                first_play = False
            ...
            # ROS2: 현재 joint 상태 publish
            joint_node.publish_joint_state()

            # ROS2 콜백 처리 (추가적인 sub 사용 시)
